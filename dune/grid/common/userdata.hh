...
namespace Dune
{
  ...
  public:
    ...
    virtual std::string name() const { return std::string("GridUserDataIF"); }
    virtual std::type_index typeIndex () const { return std::type_index(typeid(this)); }

    template <class Implementation>
    bool compare() const
    {
      return std::type_index(typeid(Implementation)) == this->typeIndex();
    }
  ...
  public:
    ...
    std::type_index typeIndex () const override { return std::type_index(typeid(Implementation)); }
